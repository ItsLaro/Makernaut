...
class SparkDev(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.mobiledev_emails = create_list_from_file("mobiledev.txt")
        self.robotics_emails = create_list_from_file("robotics.txt")
        self.underline_emails = create_list_from_file("underline.txt")
        self.vr_emails = create_list_from_file("vr.txt")
        self.webdev_emails = create_list_from_file("webdev.txt")
    ...
    async def teamup(self, ctx, email=None):
        ...
        if ctx.channel.id == self.CHECKIN_CHANNEL_ID:
            ...
            roles = [ai_role, cybersecurity_role, dig_role, gamedev_role, iot_role, it_role, mobiledev_role, robotics_role, underline_role, vr_role, webdev_role]
            ...
            if lower_email in self.ai_emails:
                ...
                await ctx.author.dm_channel.send(f'{ctx.author.mention} you now have **{str(team_role)}** role!')
                return
            elif lower_email in self.mobiledev_emails:
                team_role = mobiledev_role
                await ctx.author.add_roles(team_role)
                await ctx.author.add_roles(sparkdev_role)
                await ctx.author.create_dm()
                await ctx.author.dm_channel.send(f'{ctx.author.mention} you now have **{str(team_role)}** role!')
                ...
                await ctx.author.dm_channel.send(f'{ctx.author.mention} you now have **{str(team_role)}** role!')
                return
            elif lower_email in self.underline_emails:
                team_role = underline_role
                await ctx.author.add_roles(team_role)
                await ctx.author.add_roles(sparkdev_role)
                await ctx.author.create_dm()
                await ctx.author.dm_channel.send(f'{ctx.author.mention} you now have **{str(team_role)}** role!')
                ...
                await ctx.author.dm_channel.send(f'{ctx.author.mention} you now have **{str(team_role)}** role!')
                return
            elif lower_email in self.webdev_emails:
                team_role = webdev_role
                await ctx.author.add_roles(team_role)
                await ctx.author.add_roles(sparkdev_role)
                await ctx.author.create_dm()
                await ctx.author.dm_channel.send(f'{ctx.author.mention} you now have **{str(team_role)}** role!')
