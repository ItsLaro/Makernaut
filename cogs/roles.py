...
class Roles(commands.GroupCog, name="roles"):
    ...
    async def wash(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if mod_role not in roles:
            ...
            response = interaction.response.send_message(embed=embed_response, ephemeral=True)
    ...
    @app_commands.command(name="distribute", description="Used to add the same role to multiple users.")
        ...
        if is_success:
            ...
            if has_succesful_users:
                ...
                for successful_user in succesful_users:
                    embed_response.add_field(name="Succesful on:" if is_first == True else None, value=successful_user.mention, inline=False)
                    is_first = False
            if has_failed_users:
                is_first = True
                for failed_user in failed_users:
                    embed_response.add_field(name="Failed on:" if is_first == True else None, value=failed_user.mention, inline=False)
                    is_first = False
    ...
    async def swap(self, interaction: discord.Interaction, old_role: discord.User, new_role: discord.User):
        ...
        if is_success:
            ...
            if has_failures:
                ...
                embed_color = self.YELLOW_HEX 
                embed_response = discord.Embed(title=response_title, description=response_description, color=embed_color)
                embed_response.add_field(name="Succesful On:", value=f'{counter} users', inline=False)
